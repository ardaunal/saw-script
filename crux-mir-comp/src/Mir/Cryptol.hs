...
loadString ::
    ...
    RegValue sym (MirSlice (BVType 8)) ->
    String ->
    OverrideSim (Model sym) sym MIR rtp a r Text
...
data LoadedCryptolFunc sym = forall args ret . LoadedCryptolFunc
    ...
    , _lcfRun :: forall rtp r. OverrideSim (Model sym) sym MIR rtp args r (RegValue sym ret)
    }
...
loadCryptolFunc ::
    ...
    Text ->
    Text ->
    OverrideSim (Model sym) sym MIR rtp a r (LoadedCryptolFunc sym)
...
cryptolRun ::
    forall sym t st fs rtp r args ret .
    (IsSymInterface sym, sym ~ W4.ExprBuilder t st fs) =>
    SAW.SharedContext ->
    SAW.SAWCoreState t ->
    String ->
    ...
    TypeShape ret ->
    SAW.Term ->
    OverrideSim (Model sym) sym MIR rtp args r (RegValue sym ret)
...
exprToTerm :: forall sym t st fs tp rtp a r.
    (IsSymInterface sym, sym ~ W4.ExprBuilder t st fs) =>
    sym ->
    SAW.SharedContext ->
    SAW.SAWCoreState t ->
    ...
    W4.Expr t tp ->
    OverrideSim (Model sym) sym MIR rtp a r SAW.Term
...
regToTerm :: forall sym t st fs tp rtp a r.
    (IsSymInterface sym, sym ~ W4.ExprBuilder t st fs) =>
    sym ->
    SAW.SharedContext ->
    SAW.SAWCoreState t ->
